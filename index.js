...
app.post("/webhook", async (req, res) => {
    ...
    try { // START OF MAIN TRY BLOCK FOR WEBHOOK
        ...
        if (intent === "Booking Intent") {
            ...
            let bookingTimeStr;
            // Only attempt to parse time if rawBookingTime is a valid string
            if (typeof rawBookingTime === 'string' && rawBookingTime.includes('T') && rawBookingTime.includes('+')) {
                ...
                // Fallback for cases where rawBookingTime is not a standard timestamp string
            ...
            // --- CRUCIAL LOGIC: Check if date/time are missing and prompt from webhook ---
            if (!rawBookingDate || !rawBookingTime) {
                let fulfillmentText = "What date would you like the booking for?";
                if (!rawBookingDate && !rawBookingTime && guestCount) {
                    fulfillmentText = `Okay, for ${guestCount} guests. On what date and time would you like to book?`;
                } else if (!rawBookingDate) {
                    fulfillmentText = "What date would you like the booking for?";
                } else if (!rawBookingTime) {
                    fulfillmentText = "And what time?";
                }

                return res.json({
                    fulfillmentText: await generateGeminiReply(fulfillmentText),
                    outputContexts: [
                        {
                            ...
                            parameters: {
                                guestCount,
                                type: guestCount <= 10 ? "table" : "group"
                            },
                        },
                        {
            ...
            // If date and time are present, proceed with the original flow
